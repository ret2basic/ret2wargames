...
contract Oiler is ERC20 {
    ...
    function healthFactor(address _user) public returns (uint256) {
        ...
        uint256 hf = collateralValue * CF / borrowValue; // collateralValue / borrowValue * 75%
        // Includes 2 decimals
        return hf;
    }
    ...
    function getPriceToken() public returns (uint256) {
        return amm.getPriceToken0(); // @audit-issue Should use TWAP oracle
    }
    ...
    function liquidate(address _user) public {
        ...
        uint256 repayment = users[_user].borrow * 5 / 100; // @audit-issue Discount too much
